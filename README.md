...
The project takes three inconsistent source feeds and turns them into a clean, query-ready star schema. The main work was reconciling data that looks different across systems but represents the same thing - different date formats, different key formats, different null conventions - without silently guessing at values that couldn't be verified. On top of the warehouse, a set of analytical queries answers questions a bank would realistically ask: transaction trends over time, loan approval and credit risk, branch performance, account health, and deposit tiering. Also worth noting that visual schema for this project is available in 'src' folder.
...
Orphaned foreign keys surfaced, not hidden - transactions referencing accounts that don't exist show up as NULLs via LEFT JOIN, rather than being silently dropped by an INNER JOIN.

5. Advanced Analysis

Built on top of the gold layer, using both fact tables:

Change over time - monthly transaction count, active clients, and total amount, tracked from fact_transactions.
Cumulative analysis - running total transaction amount and moving average transaction size, month over month.
Loan portfolio health - approval rate and average interest rate, broken down by credit score band.
Branch performance - total accounts, total balance, and total transaction volume per branch, using dim_branches.
Account status breakdown - share of accounts that are active, dormant, frozen, or closed.
Deposit tiering - accounts segmented into balance ranges.
Data quality checks - percentage of orphaned transactions and closed accounts missing a close date.
Client report (gold.report_clients) - one row per client combining demographics, account balances, transaction activity, loan activity, tenure, and a High Value / Standard / New segment.
